    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try
		{
			...
			try
			{
				...
				if(rs.next())
				{
					out.println("Checking the student table.<br>");
					
					HttpSession session = request.getSession();
					
					session.setAttribute("uname",rs.getString(4));
					session.setAttribute("uid",rs.getInt(2));
				...
				{
					out.println("<script> alert('Invalid'); location = 'index.html'; </script>");
				}
			}
			catch(NumberFormatException e)
			{
				...
				if(rs.next())
				{
					out.println("Checking the admin table.<br>");
